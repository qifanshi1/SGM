			...
			if (path == nPath)
			{
				...
				for (size_t x = minX; x < maxX; x++)
				{
					...
					if (x_lower >= 0 && x_lower < width && disparity_right.at<float>(row, x_lower) >= minD &&
						...
						disparity.at<float>(row, x) = 0.0;//不符合左右一致性原则
				}
			}
		}
	}
	...
	int maxDiff)
{
	...
	std::vector<std::vector<int>> labels(height, std::vector<int>(width, 0));//记录每个像素的label
	std::vector<int> rtype(num_pixels, 0);//记录每个label的连通区域大小是否小于阈值
	int dstep = (int)(disparity.step / sizeof(float));//disparity里面的数据必须为float类型
